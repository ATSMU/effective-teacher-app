...
function doGet(e) {
  ...
  // GET ?action=getLessonFull&id=xxx — returns one lesson with all fields (files data + coverImage)
  if (e && e.parameter && e.parameter.action === 'getLessonFull') {
    var lessonId = e.parameter.id;
    if (!lessonId) {
      return ContentService.createTextOutput(JSON.stringify({ error: 'id required' }))
        .setMimeType(ContentService.MimeType.JSON);
    }
    var allLessons = getSheetData('Lessons');
    var found = null;
    for (var li = 0; li < allLessons.length; li++) {
      if (String(allLessons[li].id) === String(lessonId)) { found = allLessons[li]; break; }
    }
    return ContentService.createTextOutput(JSON.stringify({ lesson: found }))
      .setMimeType(ContentService.MimeType.JSON);
  }

  // GET ?action=getSubmissionFull&userId=xxx&taskId=xxx — returns one submission with files
  if (e && e.parameter && e.parameter.action === 'getSubmissionFull') {
    var uid = e.parameter.userId;
    var tid = e.parameter.taskId;
    var allSubs = getSheetData('Submissions');
    var sub = null;
    for (var si = 0; si < allSubs.length; si++) {
      if (String(allSubs[si].userId) === String(uid) && String(allSubs[si].taskId) === String(tid)) {
        sub = allSubs[si]; break;
      }
    }
    return ContentService.createTextOutput(JSON.stringify({ submission: sub }))
      .setMimeType(ContentService.MimeType.JSON);
  }

  // GET ?light=1 — strips heavy binary fields to speed up initial page load.
  // Lessons: files[].data stripped (metadata kept), coverImage kept for Dashboard thumbnails.
  // Submissions: files[] stripped entirely (load on demand via getSubmissionFull).
  var light = e && e.parameter && e.parameter.light === '1';
  ...
  Object.keys(SCHEMA).forEach(sheetName => {
    ...
    if (light) {
      if (sheetName === 'Lessons') {
        rows = rows.map(function(lesson) {
          var lightFiles = Array.isArray(lesson.files)
            ? lesson.files.map(function(f) {
                // Keep data only for links (URL strings, not base64), strip binary content
                return { name: f.name, size: f.size, type: f.type, isLink: !!f.isLink, data: f.isLink ? f.data : '' };
              })
            : [];
          return {
            id: lesson.id,
      ...
      if (sheetName === 'Submissions') {
        rows = rows.map(function(s) {
          return { userId: s.userId, taskId: s.taskId, response: s.response,
                   status: s.status, timestamp: s.timestamp, submitted: s.submitted, files: [] };
        });
      }
    }
    results[sheetName.toLowerCase()] = rows;
  });
