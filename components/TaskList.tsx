  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-slate-900">Задания</h2>
          <p className="text-slate-500">Список самостоятельных заданий для выполнения</p>
        </div>
        {isAdmin && (
          <button 
            onClick={onCreateNew}
      ...
      {(!tasks || tasks.length === 0) ? (
        <div className="bg-white border-2 border-dashed border-slate-200 rounded-2xl p-12 text-center">
          ...
          <h3 className="text-lg font-semibold text-slate-900">Заданий пока нет</h3>
          <p className="text-slate-500 mb-6">Как только задания будут назначены, они появятся здесь</p>
          {isAdmin && (
            <button 
              onClick={onCreateNew}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {tasks.map((task) => (
            (() => {
              ...
              return (
            <div key={task.id} className="group bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-all p-6 flex flex-col h-full overflow-hidden">
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-2">
                  <span className="px-2 py-1 rounded-lg bg-emerald-50 text-emerald-700 text-[10px] font-bold uppercase tracking-wider">Задание</span>
                  {!isAdmin && statusLabel && (
                    <span className={`px-2 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider ${avgGrade ? 'bg-indigo-50 text-indigo-600' : 'bg-amber-50 text-amber-600'}`}>
                      {statusLabel}
                    </span>
                  )}
                </div>
                {isAdmin && (
                  <div className="flex gap-1">
                    <button onClick={() => onEditTask(task)} className="p-1.5 text-slate-400 hover:text-[#10408A]" title="Редактировать">
              ...
              <h3 className="text-lg font-bold text-slate-900 mb-2 group-hover:text-[#10408A] transition-colors">{task.title}</h3>
              <p className="text-slate-500 text-sm line-clamp-4 flex-1 mb-6">
              ...
              {!isAdmin && avgGrade !== null && (
                <div className="text-xs font-black uppercase tracking-widest text-slate-400 mb-4">
                  Оценка: <span className="text-slate-900">{avgGrade} из 5</span>
                </div>
              )}
              ...
              <div className="mt-auto space-y-4">
                <button 
                  ...
                  className="w-full py-2.5 bg-[#10408A]/10 text-[#10408A] rounded-xl text-sm font-bold hover:bg-[#10408A] hover:text-white transition-all active:scale-95"
                >
                  {isAdmin ? 'Просмотреть →' : 'Выполнить →'}
                </button>
                <div className="pt-4 border-t border-slate-50 text-[10px] text-slate-400 font-medium">
