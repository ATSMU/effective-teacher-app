...
const ValidationView: React.FC<ValidationViewProps> = ({ lessons, lessonResults, onUpdateQuestion }) => {
  ...
  const renderSection = (title: string, list: ValidationItem[]) => (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs font-black text-slate-400 uppercase tracking-widest">{list.length} шт.</span>
      </div>
      {list.length === 0 ? (
        <div className="bg-white p-6 rounded-2xl border border-slate-100 text-slate-400 text-sm font-bold">
          Нет вопросов по выбранным фильтрам.
        </div>
      ) : (
        <div className="grid gap-3">
          {list.map(item => (
            <div key={item.key} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col gap-3">
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0">
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{item.lessonTitle}</p>
                  <p className="text-sm sm:text-base font-bold text-slate-900 mt-1 break-words">{item.question.text}</p>
                </div>
                <button
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="px-3 py-1 text-[10px] font-black uppercase tracking-widest rounded-full bg-slate-100 text-slate-600">
                ...
                {item.needsMoreData && (
                  <span className="px-3 py-1 text-[10px] font-black uppercase tracking-widest rounded-full bg-slate-50 text-slate-400">
                    Недостаточно данных
                  </span>
                )}
              </div>
  ...
  return (
    <div className="space-y-6 sm:space-y-8 animate-in fade-in duration-500 pb-20">
      <div>
        <h2 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">Валидация тестов</h2>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-6">
        <div className="bg-white p-4 sm:p-6 rounded-[24px] sm:rounded-[32px] border border-slate-200 shadow-sm">
          <p className="text-[8px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Легкие</p>
          <p className="text-xl sm:text-3xl font-black text-emerald-600">{easyItems.length}</p>
        </div>
        <div className="bg-white p-4 sm:p-6 rounded-[24px] sm:rounded-[32px] border border-slate-200 shadow-sm">
          <p className="text-[8px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Хорошие</p>
          <p className="text-xl sm:text-3xl font-black text-amber-600">{goodItems.length}</p>
        </div>
        <div className="bg-white p-4 sm:p-6 rounded-[24px] sm:rounded-[32px] border border-slate-200 shadow-sm">
          <p className="text-[8px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Сложные</p>
          <p className="text-xl sm:text-3xl font-black text-rose-600">{hardItems.length}</p>
        </div>
      </div>
      ...
      <div className="bg-white p-4 sm:p-6 rounded-[24px] sm:rounded-[32px] border border-slate-200 shadow-sm flex flex-col md:flex-row gap-3 sm:gap-4">
        <div className="relative w-full md:flex-1">
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-slate-50 rounded-xl sm:rounded-2xl border border-transparent focus:bg-white focus:ring-2 focus:ring-[#10408A] outline-none transition-all font-medium text-sm"
          />
        ...
        <div className="flex gap-2 w-full md:w-auto">
          <select
            ...
            className="flex-1 md:flex-none px-4 py-2.5 bg-slate-50 rounded-xl text-[10px] sm:text-sm font-bold text-slate-600 outline-none focus:bg-white border-none"
          >
